...
resource "kubernetes_secret_v1" "tap_run_cluster_trusted_ca" {
  metadata {
    name      = "${local.tap_run_cluster_name}-user-trusted-ca-secret"
...
resource "tanzu-mission-control_tanzu_kubernetes_cluster" "tap_run" {
  name                    = local.tap_run_cluster_name
  ...
  depends_on = [kubernetes_secret_v1.tap_run_cluster_trusted_ca]
...
resource "tanzu-mission-control_git_repository" "tap_run_installation" {
  ...
  spec {
    ...
    secret_ref         = local.git_requires_credentials ? tanzu-mission-control_repository_credential.tap_run_cluster_repo[0].name : null
    interval           = "60s"
  ...
  depends_on = [tanzu-mission-control_repository_credential.tap_run_cluster_repo]
}

resource "tanzu-mission-control_kustomization" "tap_run_cluster" {
  name = "tap-gitops"
  ...
  spec {
    ...
    source {
      name      = tanzu-mission-control_git_repository.tap_run_installation.name
      namespace = tanzu-mission-control_git_repository.tap_run_installation.namespace_name
    }
  }

  depends_on = [tanzu-mission-control_git_repository.tap_run_installation]
}
