...
resource "kubernetes_secret_v1" "tap_iterate_cluster_trusted_ca" {
  metadata {
    name      = "${local.tap_iterate_cluster_name}-user-trusted-ca-secret"
...
resource "tanzu-mission-control_tanzu_kubernetes_cluster" "tap_iterate" {
  name                    = local.tap_iterate_cluster_name
  ...
  depends_on = [kubernetes_secret_v1.tap_iterate_cluster_trusted_ca]
...
resource "kubernetes_secret_v1" "tap_iterate_cluster_age_key" {
  provider = kubernetes.tap_iterate_cluster
...
resource "tanzu-mission-control_repository_credential" "tap_iterate_cluster_repo" {
  ...
  scope {
    cluster {
      name                    = tanzu-mission-control_tanzu_kubernetes_cluster.tap_iterate.name
      provisioner_name        = tanzu-mission-control_tanzu_kubernetes_cluster.tap_iterate.provisioner_name
      management_cluster_name = tanzu-mission-control_tanzu_kubernetes_cluster.tap_iterate.management_cluster_name
    }
  }
...
resource "tanzu-mission-control_git_repository" "tap_iterate_installation" {
  ...
  spec {
    ...
    secret_ref         = local.git_requires_credentials ? tanzu-mission-control_repository_credential.tap_iterate_cluster_repo[0].name : null
    interval           = "60s"
  ...
  depends_on = [tanzu-mission-control_repository_credential.tap_iterate_cluster_repo]
}

resource "tanzu-mission-control_kustomization" "tap_iterate_cluster" {
  name = "tap-gitops"
  ...
  spec {
    ...
    path     = "manifests/flux-tap/multi-cluster/overlays/iterate"
    prune    = true
    interval = "60s"

    source {
      name      = tanzu-mission-control_git_repository.tap_iterate_installation.name
      namespace = tanzu-mission-control_git_repository.tap_iterate_installation.namespace_name
    }
  }

  depends_on = [tanzu-mission-control_git_repository.tap_iterate_installation]
}
